...
- Aレコード照会（`dns.resolve4`）によるDNS解決
- ホワイトリストはブランドごとに管理
- 同一組織内の他ブランドの保有ドメインも除外対象
:::
...
ThreatGuard は以下のcronスケジュールで自動スキャンを実行します。

| ジョブ | スケジュール | 説明 |
|--------|-----------|------|
| **メインスキャン** | 6時間ごと（`0 */6 * * *`） | 全ブランドのCT監視 + 類似ドメイン生成スキャン |
| **Webプローブ** | 1日4回（3:00, 9:00, 15:00, 21:00） | 未解決ドメインの状態変化を自動検出 |
| **Twitter監視** | 1日6回（1:00, 5:00, 9:00, 13:00, 17:00, 21:00） | フィッシングURL拡散の監視 |
| **クリーンアップ** | 毎日4:00 | 期限切れの無料診断結果の自動削除 |

::: tip メインスキャンの間隔
メインスキャンの実行間隔は環境変数 `SCAN_CRON` でカスタマイズ可能です。
:::
